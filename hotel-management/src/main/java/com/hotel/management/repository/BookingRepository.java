...
public interface BookingRepository extends JpaRepository<Booking, Integer> {
    ...
    @Query("SELECT b FROM Booking b WHERE " +
           ...
           "(:startDate IS NULL OR b.checkInAt >= :startDate) AND " +
           "(:endDate IS NULL OR b.checkOutAt <= :endDate)")
    Page<Booking> filterBookings(@Param("userId") Integer userId,
                                 @Param("roomId") Integer roomId,
                                 @Param("status") BookingStatus status,
                                 @Param("startDate") LocalDateTime startDate,
                                 @Param("endDate") LocalDateTime endDate,
    ...
    /**
     * Check if a room has a conflicting booking in the given time range.
     * Overlap condition: checkIn < existingCheckOut AND checkOut > existingCheckIn
     * Only considers bookings with status CONFIRMED or CHECKED_IN.
     *
     * @param roomId    the room ID to check
     * @param checkIn   the requested check-in time
     * @param checkOut  the requested check-out time
     * @param excludeId booking ID to exclude (for updates), pass null to include all
    ...
    /**
     * Count bookings created today (from midnight to now).
     */
    ...
    /**
     * Count bookings currently CHECKED_IN (active guests).
     */
